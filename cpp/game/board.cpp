...
Color Board::checkWinner() const
{
  ...
  for (int x = 0; x < x_size; x++)
  {
    Loc startLoc = Location::getLoc(x, 0, x_size);
    if (colors[startLoc] == C_BLACK)
    {
      if (isWinHelper(visited, startLoc, C_BLACK))return C_BLACK;
    }
  }
  ...
  for (int y = 0; y < y_size; y++)
  {
    Loc startLoc = Location::getLoc(0, y, x_size);
    if (colors[startLoc] == C_WHITE)
    {
      if (isWinHelper(visited, startLoc, C_WHITE))return C_WHITE;
    }
  }
  return C_EMPTY;
}

bool Board::isWinHelper(bool* visited, Loc startLoc,Color color) const
{
  if(colors[startLoc]!=color)return false;
  if(visited[startLoc])return false;
  visited[startLoc] = true;
  if (color == C_BLACK && Location::getY(startLoc, x_size) == y_size - 1)return true;
  if (color == C_WHITE && Location::getX(startLoc, x_size) == x_size - 1)return true;
  for (int i = 0; i < 6; i++)
    if (isWinHelper(visited, startLoc + adj_offsets[i], color))return true;
  return false;
}
